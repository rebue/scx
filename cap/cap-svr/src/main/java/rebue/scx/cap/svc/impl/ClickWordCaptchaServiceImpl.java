...
public class ClickWordCaptchaServiceImpl extends AbstractCaptchaService {
    ...
    public void init(final Properties config) {
        ...
        try {
            final int size = Integer.valueOf(config.getProperty(Const.CAPTCHA_FONT_SIZE, HAN_ZI_SIZE + ""));
            ...
            if (clickWordFontStr.toLowerCase().endsWith(".ttf")
                ...
                clickWordFont = Font.createFont(Font.TRUETYPE_FONT,
                        getClass().getResourceAsStream("/fonts/" + clickWordFontStr))
                        .deriveFont(Font.BOLD, size);
            }
            else {
                final int style = Integer.valueOf(config.getProperty(Const.CAPTCHA_FONT_STYLE, Font.BOLD + ""));
                clickWordFont = new Font(clickWordFontStr, style, size);
            }
        } catch (final Exception ex) {
            logger.error("load font error:{}", ex);
        }
        wordTotalCount = Integer.valueOf(config.getProperty(Const.CAPTCHA_WORD_COUNT, "4"));
    }
    ...
    public Ro<?> check(final CaptchaVO captchaVO) {
        ...
        // 校验成功，将信息存入缓存
        final String secretKey = point.get(0).getSecretKey();
        String       value     = null;
        try {
    ...
    public void setWordTotalCount(final int wordTotalCount) {
    ...
    public void setFontColorRandom(final boolean fontColorRandom) {
    ...
    private CaptchaVO getImageData(final BufferedImage backgroundImage) {
        ...
        final int           wordCount          = getWordTotalCount();
        // 定义随机1到arr.length某一个字不参与校验
        final int           num                = RandomUtils.getRandomInt(1, wordCount);
        final Set<String>   currentWords       = getRandomWords(wordCount);
         ...
         */
        int i = 0;
        for (final String word : currentWords) {
            // 随机字体坐标
            final PointVO point = randomWordPoint(width, height, i, wordCount);
            ...
            final Font rotatedFont = clickWordFont.deriveFont(affineTransform);
        ...
        // 创建合并图片
        final BufferedImage combinedImage    = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
    ...
    private Set<String> getRandomWords(final int wordCount) {
        final Set<String> words = new HashSet<>();
        final int         size  = HAN_ZI.length();
        for (;;) {
            final String t = HAN_ZI.charAt(RandomUtils.getRandomInt(size)) + "";
            words.add(t);
     ...
     * @return
     */
    private static PointVO randomWordPoint(final int imageWidth, final int imageHeight, final int wordSortIndex, final int wordCount) {
        final int avgWidth = imageWidth / (wordCount + 1);
        int       x, y;
