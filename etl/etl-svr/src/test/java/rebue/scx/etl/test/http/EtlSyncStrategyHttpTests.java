 ...
 */
@Slf4j
public class EtlSyncStrategyHttpTests {
    ...
    public void testCrud() throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/etl/sync-strategy", addTo);
            log.info("添加同步策略的返回值为：" + addResult);
        ...
        final String pageResult = _httpClient.get(_hostUrl + "/etl/sync-strategy/page");
        log.info("查询同步策略的返回值为：" + pageResult);
        final Ro<PageRa<EtlSyncStrategyMo>> pageRo = JacksonUtils.deserialize(pageResult, new TypeReference<Ro<PageRa<EtlSyncStrategyMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, pageRo.getResult());
        log.info("获取单个同步策略的参数为：" + id);
        final String getByIdResult = _httpClient.get(_hostUrl + "/etl/sync-strategy/get-by-id?id=" + id);
        log.info("获取单个同步策略的返回值为：" + getByIdResult);
        final EtlSyncStrategyModifyTo modifyTo = _dozerMapper.map(addTo, EtlSyncStrategyModifyTo.class);
        modifyTo.setId(id);
        log.info("修改同步策略的参数为：" + modifyTo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/etl/sync-strategy", modifyTo);
        final Ro<PojoRa<EtlSyncStrategyMo>> getByIdRo = JacksonUtils.deserialize(getByIdResult, new TypeReference<Ro<PojoRa<EtlSyncStrategyMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, getByIdRo.getResult());
        EtlSyncStrategyMo mo = getByIdRo.getExtra().getOne();
        ...
        log.info("删除同步策略的参数为：" + mo.getId());
        final String deleteResult = _httpClient.delete(_hostUrl + "/etl/sync-strategy?id=" + mo.getId());
