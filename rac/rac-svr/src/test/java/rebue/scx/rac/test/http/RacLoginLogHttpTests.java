 ...
 */
@Slf4j
public class RacLoginLogHttpTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/login-log", mo);
            log.info("添加用户登录日志的返回值为：" + addResult);
        ...
        final String listResult = _httpClient.get(_hostUrl + "/rac/login-log/list?pageNum=1&pageSize=5");
        log.info("查询用户登录日志的返回值为：" + listResult);
        log.info("获取单个用户登录日志的参数为：" + mo.getId());
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/login-log/get-by-id?id=" + mo.getId());
        log.info("获取单个用户登录日志的返回值为：" + getByIdResult);
        log.info("修改用户登录日志的参数为：" + mo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/login-log", mo);
        log.info("修改用户登录日志的返回值为：" + modifyResult);
        final Ro modifyRo = _objectMapper.readValue(modifyResult, Ro.class);
        log.info(modifyRo.toString());
        Assertions.assertEquals(ResultDic.SUCCESS, modifyRo.getResult());
        log.info("删除用户登录日志的参数为：" + mo.getId());
        final String deleteResult = _httpClient.delete(_hostUrl + "/rac/login-log?id=" + mo.getId());
