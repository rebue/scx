 ...
 */
@Slf4j
public class RacUserRoleHttpTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/user-role", mo);
            log.info("添加用户角色的返回值为：" + addResult);
        ...
        final String listResult = _httpClient.get(_hostUrl + "/rac/user-role/list?pageNum=1&pageSize=5");
        log.info("查询用户角色的返回值为：" + listResult);
        log.info("获取单个用户角色的参数为：" + mo.getId());
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/user-role/get-by-id?id=" + mo.getId());
        log.info("获取单个用户角色的返回值为：" + getByIdResult);
        log.info("修改用户角色的参数为：" + mo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/user-role", mo);
        log.info("修改用户角色的返回值为：" + modifyResult);
        final Ro modifyRo = _objectMapper.readValue(modifyResult, Ro.class);
        log.info(modifyRo.toString());
        Assertions.assertEquals(ResultDic.SUCCESS, modifyRo.getResult());
        log.info("删除用户角色的参数为：" + mo.getId());
        final String deleteResult = _httpClient.delete(_hostUrl + "/rac/user-role?id=" + mo.getId());
