...
public class RacPermUrnHttpTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/perm-urn", mo);
        ...
        final String listResult = _httpClient.get(_hostUrl + "/rac/perm-urn/list?pageNum=1&pageSize=5");
        log.info("查询权限URN的返回值为：" + listResult);
        log.info("获取单个权限URN的参数为：" + mo.getId());
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/perm-urn/get-by-id?id=" + mo.getId());
        log.info("获取单个权限URN的返回值为：" + getByIdResult);
        log.info("修改权限URN的参数为：" + mo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/perm-urn", mo);
