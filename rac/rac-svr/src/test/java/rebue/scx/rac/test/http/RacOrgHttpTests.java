...
public class RacOrgHttpTests {
    ...
    public void testCrud() throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/org", addTo);
            log.info("添加组织的返回值为：" + addResult);
        ...
        final String pageResult = _httpClient.get(_hostUrl + "/rac/org/page");
        log.info("查询组织的返回值为：" + pageResult);
        final Ro<PageRa<RacOrgMo>> pageRo = JacksonUtils.deserialize(pageResult, new TypeReference<Ro<PageRa<RacOrgMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, pageRo.getResult());
        log.info("获取单个组织的参数为：" + id);
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/org/get-by-id?id=" + id);
        log.info("获取单个组织的返回值为：" + getByIdResult);
        final RacOrgModifyTo modifyTo = _dozerMapper.map(addTo, RacOrgModifyTo.class);
        modifyTo.setId(id);
        log.info("修改组织的参数为：" + modifyTo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/org", modifyTo);
        final Ro<PojoRa<RacOrgMo>> getByIdRo = JacksonUtils.deserialize(getByIdResult, new TypeReference<Ro<PojoRa<RacOrgMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, getByIdRo.getResult());
        RacOrgMo mo = getByIdRo.getExtra().getOne();
