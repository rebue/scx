...
@Slf4j
@SpringBootTest
public class RacDomainApiTests {
    ...
    public void testCrud() {
        ...
        final Ro<PageRa<RacDomainMo>> pageResult = _api.page(new RacDomainPageTo());
        log.info("查询领域的返回值为：" + pageResult);
        Assertions.assertEquals(ResultDic.SUCCESS, pageResult.getResult());
        log.info("获取单个领域的参数为：" + id);
        final Ro<PojoRa<RacDomainMo>> getByIdResult = _api.getById(id);
