...
public class RacPermMenuHttpTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/perm-menu", mo);
        ...
        final String listResult = _httpClient.get(_hostUrl + "/rac/perm-menu/list?pageNum=1&pageSize=5");
        log.info("查询权限菜单的返回值为：" + listResult);
        log.info("获取单个权限菜单的参数为：" + mo.getId());
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/perm-menu/get-by-id?id=" + mo.getId());
        log.info("获取单个权限菜单的返回值为：" + getByIdResult);
        log.info("修改权限菜单的参数为：" + mo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/perm-menu", mo);
