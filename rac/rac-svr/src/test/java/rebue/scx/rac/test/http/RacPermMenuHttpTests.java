 ...
 */
@Slf4j
public class RacPermMenuHttpTests {
    ...
    public void testCrud() throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/perm-menu", addTo);
            log.info("添加权限菜单的返回值为：" + addResult);
        ...
        final String pageResult = _httpClient.get(_hostUrl + "/rac/perm-menu/page");
        log.info("查询权限菜单的返回值为：" + pageResult);
        final Ro<PageRa<RacPermMenuMo>> pageRo = JacksonUtils.deserialize(pageResult, new TypeReference<Ro<PageRa<RacPermMenuMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, pageRo.getResult());
        log.info("获取单个权限菜单的参数为：" + id);
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/perm-menu/get-by-id?id=" + id);
        log.info("获取单个权限菜单的返回值为：" + getByIdResult);
        final RacPermMenuModifyTo modifyTo = _dozerMapper.map(addTo, RacPermMenuModifyTo.class);
        modifyTo.setId(id);
        log.info("修改权限菜单的参数为：" + modifyTo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/perm-menu", modifyTo);
        final Ro<PojoRa<RacPermMenuMo>> getByIdRo = JacksonUtils.deserialize(getByIdResult, new TypeReference<Ro<PojoRa<RacPermMenuMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, getByIdRo.getResult());
        RacPermMenuMo mo = getByIdRo.getExtra().getOne();
        ...
        final String deleteResult = _httpClient.delete(_hostUrl + "/rac/perm-menu?id=" + mo.getId());
