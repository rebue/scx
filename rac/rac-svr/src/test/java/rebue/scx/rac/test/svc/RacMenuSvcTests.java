...
public class RacMenuSvcTests {
    ...
    public void testCrud() {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final Ro<IdRa<String>> idRo = _svc.add(mo);
            log.info("添加菜单信息的返回值为：" + idRo);
            Assertions.assertEquals(ResultDic.SUCCESS, idRo.getResult());
            mo.setId(idRo.getExtra().getId());
        }
        final Ro<PageRa<RacMenuMo>> listResult = _svc.list(null, 1, 5, null, 10);
        ...
        final Ro<PojoRa<RacMenuMo>> getByIdResult = _svc.getById(mo.getId());
