...
public class RacRoleHttpTests {
    ...
    public void testCrud() throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/role", addTo);
            log.info("添加角色的返回值为：" + addResult);
        ...
        final String pageResult = _httpClient.get(_hostUrl + "/rac/role/page");
        log.info("查询角色的返回值为：" + pageResult);
        final Ro<PageRa<RacRoleMo>> pageRo = JacksonUtils.deserialize(pageResult, new TypeReference<Ro<PageRa<RacRoleMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, pageRo.getResult());
        log.info("获取单个角色的参数为：" + id);
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/role/get-by-id?id=" + id);
        log.info("获取单个角色的返回值为：" + getByIdResult);
        final RacRoleModifyTo modifyTo = _dozerMapper.map(addTo, RacRoleModifyTo.class);
        modifyTo.setId(id);
        log.info("修改角色的参数为：" + modifyTo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/role", modifyTo);
        final Ro<PojoRa<RacRoleMo>> getByIdRo = JacksonUtils.deserialize(getByIdResult, new TypeReference<Ro<PojoRa<RacRoleMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, getByIdRo.getResult());
        RacRoleMo mo = getByIdRo.getExtra().getOne();
