 ...
 */
@Slf4j
public class RacDomainHttpTests {
    ...
    public void testCrud() throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/domain", addTo);
        ...
        final String pageResult = _httpClient.get(_hostUrl + "/rac/domain/page");
        log.info("查询领域的返回值为：" + pageResult);
        final Ro<PageRa<RacDomainMo>> pageRo = JacksonUtils.deserialize(pageResult, new TypeReference<Ro<PageRa<RacDomainMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, pageRo.getResult());
        log.info("获取单个领域的参数为：" + id);
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/domain/get-by-id?id=" + id);
        log.info("获取单个领域的返回值为：" + getByIdResult);
        final RacDomainModifyTo modifyTo = _dozerMapper.map(addTo, RacDomainModifyTo.class);
        modifyTo.setId(id);
        log.info("修改领域的参数为：" + modifyTo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/domain", modifyTo);
        final Ro<PojoRa<RacDomainMo>> getByIdRo = JacksonUtils.deserialize(getByIdResult, new TypeReference<Ro<PojoRa<RacDomainMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, getByIdRo.getResult());
        RacDomainMo mo = getByIdRo.getExtra().getOne();
        ...
        final String deleteResult = _httpClient.delete(_hostUrl + "/rac/domain?id=" + mo.getId());
