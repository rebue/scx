...
public class RacUserHttpTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/user", mo);
            log.info("添加用户信息的返回值为：" + addResult);
        ...
        final String listResult = _httpClient.get(_hostUrl + "/rac/user/list?pageNum=1&pageSize=5");
        log.info("查询用户信息的返回值为：" + listResult);
        log.info("获取单个用户信息的参数为：" + mo.getId());
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/user/get-by-id?id=" + mo.getId());
        log.info("获取单个用户信息的返回值为：" + getByIdResult);
        log.info("修改用户信息的参数为：" + mo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/user", mo);
