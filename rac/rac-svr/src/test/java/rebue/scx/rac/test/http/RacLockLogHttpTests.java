 ...
 */
@Slf4j
public class RacLockLogHttpTests {
    ...
    public void testCrud() throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/lock-log", addTo);
            log.info("添加锁定日志的返回值为：" + addResult);
        ...
        final String pageResult = _httpClient.get(_hostUrl + "/rac/lock-log/page");
        log.info("查询锁定日志的返回值为：" + pageResult);
        final Ro<PageRa<RacLockLogMo>> pageRo = JacksonUtils.deserialize(pageResult, new TypeReference<Ro<PageRa<RacLockLogMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, pageRo.getResult());
        log.info("获取单个锁定日志的参数为：" + id);
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/lock-log/get-by-id?id=" + id);
        log.info("获取单个锁定日志的返回值为：" + getByIdResult);
        final RacLockLogModifyTo modifyTo = _dozerMapper.map(addTo, RacLockLogModifyTo.class);
        modifyTo.setId(id);
        log.info("修改锁定日志的参数为：" + modifyTo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/lock-log", modifyTo);
        final Ro<PojoRa<RacLockLogMo>> getByIdRo = JacksonUtils.deserialize(getByIdResult, new TypeReference<Ro<PojoRa<RacLockLogMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, getByIdRo.getResult());
        RacLockLogMo mo = getByIdRo.getExtra().getOne();
        ...
        log.info("删除锁定日志的参数为：" + mo.getId());
        final String deleteResult = _httpClient.delete(_hostUrl + "/rac/lock-log?id=" + mo.getId());
