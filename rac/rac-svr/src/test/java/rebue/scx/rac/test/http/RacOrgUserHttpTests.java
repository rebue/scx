 ...
 */
@Slf4j
public class RacOrgUserHttpTests {
    ...
    public void testCrud() throws IOException, ReflectiveOperationException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/org-user", mo);
            log.info("添加组织用户的返回值为：" + addResult);
        ...
        final String listResult = _httpClient.get(_hostUrl + "/rac/org-user/list?pageNum=1&pageSize=5");
        log.info("查询组织用户的返回值为：" + listResult);
        log.info("获取单个组织用户的参数为：" + mo.getId());
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/org-user/get-by-id?id=" + mo.getId());
        log.info("获取单个组织用户的返回值为：" + getByIdResult);
        log.info("修改组织用户的参数为：" + mo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/org-user", mo);
        log.info("修改组织用户的返回值为：" + modifyResult);
        final Ro modifyRo = _objectMapper.readValue(modifyResult, Ro.class);
        log.info(modifyRo.toString());
        Assertions.assertEquals(ResultDic.SUCCESS, modifyRo.getResult());
        log.info("删除组织用户的参数为：" + mo.getId());
        final String deleteResult = _httpClient.delete(_hostUrl + "/rac/org-user?id=" + mo.getId());
