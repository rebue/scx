 ...
 */
@Slf4j
public class RacStatusHttpTests {
    ...
    public void testCrud() throws IOException {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            final String addResult = _httpClient.postByJsonParams(_hostUrl + "/rac/status", addTo);
            log.info("添加身份的返回值为：" + addResult);
        ...
        final String pageResult = _httpClient.get(_hostUrl + "/rac/status/page");
        log.info("查询身份的返回值为：" + pageResult);
        final Ro<PageRa<RacStatusMo>> pageRo = JacksonUtils.deserialize(pageResult, new TypeReference<Ro<PageRa<RacStatusMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, pageRo.getResult());
        log.info("获取单个身份的参数为：" + id);
        final String getByIdResult = _httpClient.get(_hostUrl + "/rac/status/get-by-id?id=" + id);
        log.info("获取单个身份的返回值为：" + getByIdResult);

        final RacStatusModifyTo modifyTo = _dozerMapper.map(addTo, RacStatusModifyTo.class);
        modifyTo.setId(id);
        log.info("修改身份的参数为：" + modifyTo);
        final String modifyResult = _httpClient.putByJsonParams(_hostUrl + "/rac/status", modifyTo);
        final Ro<PojoRa<RacStatusMo>> getByIdRo = JacksonUtils.deserialize(getByIdResult, new TypeReference<Ro<PojoRa<RacStatusMo>>>() {
        });
        Assertions.assertEquals(ResultDic.SUCCESS, getByIdRo.getResult());
        RacStatusMo mo = getByIdRo.getExtra().getOne();
        ...
        log.info("删除身份的参数为：" + mo.getId());
        final String deleteResult = _httpClient.delete(_hostUrl + "/rac/status?id=" + mo.getId());
