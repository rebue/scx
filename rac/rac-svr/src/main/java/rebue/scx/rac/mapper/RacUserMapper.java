...
public interface RacUserMapper extends MapperRootInterface<RacUserMo, Long> {
    ...
    default Optional<RacUserNonDesensitizedMo> selectByKey(Long userId) {
        SelectStatementProvider select = SqlBuilder.select(racUser.allColumns()).from(racUser).where(racUser.id, isEqualTo(userId)).build().render(RenderingStrategies.MYBATIS3);
        return this.selectOneNonDesensitized(select);
    }

    default Optional<RacUserNonDesensitizedMo> selectByOne(RacUserMo record) {
        SelectStatementProvider select = SqlBuilder.select(racUser.allColumns()).from(racUser).where(id, isEqualToWhenPresent(record::getId))
            .and(mobile, isEqualToWhenPresent(record::getMobile)).and(isVerifiedMobile, isEqualToWhenPresent(record::getIsVerifiedMobile))
            .and(email, isEqualToWhenPresent(record::getEmail)).and(isVerifiedEmail, isEqualToWhenPresent(record::getIsVerifiedEmail))
            .and(realName, isEqualToWhenPresent(record::getRealName)).and(isVerifiedRealname, isEqualToWhenPresent(record::getIsVerifiedRealname))
            .and(idCard, isEqualToWhenPresent(record::getIdCard)).and(isVerifiedIdcard, isEqualToWhenPresent(record::getIsVerifiedIdcard))
            .and(sex, isEqualToWhenPresent(record::getSex)).and(createTimestamp, isEqualToWhenPresent(record::getCreateTimestamp))
