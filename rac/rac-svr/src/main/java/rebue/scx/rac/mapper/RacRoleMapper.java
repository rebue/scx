...
public interface RacRoleMapper extends MapperRootInterface<RacRoleMo, Long> {
	...
	default int insert(RacRoleMo record) {
		return MyBatis3Utils.insert(this::insert, record, racRole,
				c -> c.map(id).toProperty("id").map(name).toProperty("name").map(domainId).toProperty("domainId")
	...
	default int insertMultiple(Collection<RacRoleMo> records) {
		return MyBatis3Utils.insertMultiple(this::insertMultiple, records, racRole,
				c -> c.map(id).toProperty("id").map(name).toProperty("name").map(domainId).toProperty("domainId")
						.map(isEnabled).toProperty("isEnabled").map(seqNo).toProperty("seqNo").map(remark)
						.toProperty("remark"));
	}
	...
	default int insertSelective(RacRoleMo record) {
		return MyBatis3Utils.insert(this::insert, record, racRole,
				c -> c.map(id).toPropertyWhenPresent("id", record::getId).map(name)
						.toPropertyWhenPresent("name", record::getName).map(domainId)
	...
	static UpdateDSL<UpdateModel> updateAllColumns(RacRoleMo record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(id).equalTo(record::getId).set(name).equalTo(record::getName).set(domainId)
				.equalTo(record::getDomainId).set(isEnabled).equalTo(record::getIsEnabled).set(seqNo)
	...
	static UpdateDSL<UpdateModel> updateSelectiveColumns(RacRoleMo record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(id).equalToWhenPresent(record::getId).set(name).equalToWhenPresent(record::getName).set(domainId)
	...
	default int updateByPrimaryKey(RacRoleMo record) {
		return update(c -> c.set(name).equalTo(record::getName).set(domainId).equalTo(record::getDomainId)
	...
	default int updateByPrimaryKeySelective(RacRoleMo record) {
		return update(c -> c.set(name).equalToWhenPresent(record::getName).set(domainId)
				.equalToWhenPresent(record::getDomainId).set(isEnabled).equalToWhenPresent(record::getIsEnabled)
				.set(seqNo).equalToWhenPresent(record::getSeqNo).set(remark).equalToWhenPresent(record::getRemark)
