...
public class RacAccountCtrl {
	 ...
	 */
	@RacOpLog(opType = "修改账户密码", opTitle = "修改账户: #{#p0.id}")
	...
	public Mono<?> uploadAvatar(@CookieValue(JwtUtils.JWT_TOKEN_NAME) final String jwtToken,
			@RequestPart("avatar") final Flux<FilePart> filePartFlux, final ServerHttpResponse response) {
		if (StringUtils.isBlank(jwtToken)) {
			throw new IllegalArgumentException("在Cookie中找不到JWT签名");
		}
		...
		return filePartFlux.flatMap(filePart -> {
			...
			return filePart.content().map(dataBuffer -> dataBuffer.asInputStream(true)).reduce(SequenceInputStream::new)
					.map(inputStream -> {
						final Ro<?> ro = api.uploadAvatar(curAccountId, fileName, contentDisposition, contentType,
								inputStream);
