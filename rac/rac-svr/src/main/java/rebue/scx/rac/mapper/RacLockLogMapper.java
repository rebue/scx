...
public interface RacLockLogMapper extends MapperRootInterface<RacLockLogMo, Long> {
    ...
    default long countLockSurvey(RacLockLogPageTo record) {
        SelectStatementProvider count = SqlBuilder.countFrom(racLockLog).where(racLockLog.lockDatetime, isGreaterThanWhenPresent(record.getStartDate()))
            .and(racLockLog.lockDatetime, isLessThanWhenPresent(record.getEndDate())).build().render(RenderingStrategies.MYBATIS3);
    ...
    default Long countUnlockSurvey(RacLockLogPageTo record) {
        SelectStatementProvider count = SqlBuilder.countFrom(racLockLog).where(racLockLog.lockDatetime, isGreaterThanWhenPresent(record.getStartDate()))
            .and(racLockLog.lockDatetime, isLessThanWhenPresent(record.getEndDate())).and(racLockLog.unlockDatetime, isNotNull()).build().render(RenderingStrategies.MYBATIS3);
        return this.count(count);
    }
}
