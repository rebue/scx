...
public interface RacLockLogMapper extends MapperRootInterface<RacLockLogMo, Long> {
	 ...
	 */
	BasicColumn[] selectList = BasicColumn.columnList(id, sysId, lockAccountId, lockOpId, lockReason, lockDatetime,
			unlockReason, unlockDatetime, unlockOpId);
	...
	default int insert(RacLockLogMo record) {
		return MyBatis3Utils.insert(this::insert, record, racLockLog,
				c -> c.map(id).toProperty("id").map(sysId).toProperty("sysId").map(lockAccountId)
						.toProperty("lockAccountId").map(lockOpId).toProperty("lockOpId").map(lockReason)
						.toProperty("lockReason").map(lockDatetime).toProperty("lockDatetime").map(unlockReason)
						.toProperty("unlockReason").map(unlockDatetime).toProperty("unlockDatetime").map(unlockOpId)
						.toProperty("unlockOpId"));
	}
	...
	default int insertSelective(RacLockLogMo record) {
		return MyBatis3Utils.insert(this::insert, record, racLockLog,
				c -> c.map(id).toPropertyWhenPresent("id", record::getId).map(sysId)
						.toPropertyWhenPresent("sysId", record::getSysId).map(lockAccountId)
						.toPropertyWhenPresent("lockAccountId", record::getLockAccountId).map(lockOpId)
						.toPropertyWhenPresent("lockOpId", record::getLockOpId).map(lockReason)
						.toPropertyWhenPresent("lockReason", record::getLockReason).map(lockDatetime)
						.toPropertyWhenPresent("lockDatetime", record::getLockDatetime).map(unlockReason)
						.toPropertyWhenPresent("unlockReason", record::getUnlockReason).map(unlockDatetime)
						.toPropertyWhenPresent("unlockDatetime", record::getUnlockDatetime).map(unlockOpId)
						.toPropertyWhenPresent("unlockOpId", record::getUnlockOpId));
	}
	...
	static UpdateDSL<UpdateModel> updateAllColumns(RacLockLogMo record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(id).equalTo(record::getId).set(sysId).equalTo(record::getSysId).set(lockAccountId)
				.equalTo(record::getLockAccountId).set(lockOpId).equalTo(record::getLockOpId).set(lockReason)
				.equalTo(record::getLockReason).set(lockDatetime).equalTo(record::getLockDatetime).set(unlockReason)
				.equalTo(record::getUnlockReason).set(unlockDatetime).equalTo(record::getUnlockDatetime).set(unlockOpId)
				.equalTo(record::getUnlockOpId);
	}
	...
	static UpdateDSL<UpdateModel> updateSelectiveColumns(RacLockLogMo record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(id).equalToWhenPresent(record::getId).set(sysId).equalToWhenPresent(record::getSysId)
				.set(lockAccountId).equalToWhenPresent(record::getLockAccountId).set(lockOpId)
				.equalToWhenPresent(record::getLockOpId).set(lockReason).equalToWhenPresent(record::getLockReason)
				.set(lockDatetime).equalToWhenPresent(record::getLockDatetime).set(unlockReason)
				.equalToWhenPresent(record::getUnlockReason).set(unlockDatetime)
				.equalToWhenPresent(record::getUnlockDatetime).set(unlockOpId)
				.equalToWhenPresent(record::getUnlockOpId);
	}
	...
	default int updateByPrimaryKey(RacLockLogMo record) {
		return update(c -> c.set(sysId).equalTo(record::getSysId).set(lockAccountId).equalTo(record::getLockAccountId)
				.set(lockOpId).equalTo(record::getLockOpId).set(lockReason).equalTo(record::getLockReason)
				.set(lockDatetime).equalTo(record::getLockDatetime).set(unlockReason).equalTo(record::getUnlockReason)
				.set(unlockDatetime).equalTo(record::getUnlockDatetime).set(unlockOpId).equalTo(record::getUnlockOpId)
	...
	default int updateByPrimaryKeySelective(RacLockLogMo record) {
		return update(c -> c.set(sysId).equalToWhenPresent(record::getSysId).set(lockAccountId)
				.equalToWhenPresent(record::getLockAccountId).set(lockOpId).equalToWhenPresent(record::getLockOpId)
				.set(lockReason).equalToWhenPresent(record::getLockReason).set(lockDatetime)
				.equalToWhenPresent(record::getLockDatetime).set(unlockReason)
				.equalToWhenPresent(record::getUnlockReason).set(unlockDatetime)
				.equalToWhenPresent(record::getUnlockDatetime).set(unlockOpId).equalToWhenPresent(record::getUnlockOpId)
				.where(id, isEqualTo(record::getId)));
	}
	...
	default int deleteSelective(RacLockLogMo record) {
		return delete(
				c -> c.where(id, isEqualToWhenPresent(record::getId)).and(sysId, isEqualToWhenPresent(record::getSysId))
	...
	@Select({ "<script>"
			...
			+ "</script>" })
	List<RacLockLogExMo> selectSEx(@Param(value = "record") RacLockLogPageTo record);
}