...
public interface RacPermUrnMapper extends MapperRootInterface<RacPermUrnMo, Long> {
     ...
     */
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "RacPermUrnMoResult", value = { @Result(column = "ID", property = "id", jdbcType = JdbcType.BIGINT, id = true),
        @Result(column = "PERM_ID", property = "permId", jdbcType = JdbcType.BIGINT), @Result(column = "URN", property = "urn", jdbcType = JdbcType.VARCHAR)
    })
    ...
    default int insert(RacPermUrnMo record) {
        return MyBatis3Utils.insert(this::insert, record, racPermUrn, c -> c.map(id).toProperty("id").map(permId).toProperty("permId").map(urn).toProperty("urn"));
    }
    ...
    default int insertMultiple(Collection<RacPermUrnMo> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, racPermUrn, c -> c.map(id).toProperty("id").map(permId).toProperty("permId").map(urn).toProperty("urn"));
    }
    ...
    default int insertSelective(RacPermUrnMo record) {
        return MyBatis3Utils.insert(this::insert, record, racPermUrn, c -> c.map(id).toPropertyWhenPresent("id", record::getId).map(permId)
            .toPropertyWhenPresent("permId", record::getPermId).map(urn).toPropertyWhenPresent("urn", record::getUrn));
    }
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(RacPermUrnMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId).set(permId).equalTo(record::getPermId).set(urn).equalTo(record::getUrn);
    }
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(RacPermUrnMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId).set(permId).equalToWhenPresent(record::getPermId).set(urn).equalToWhenPresent(record::getUrn);
    }
    ...
    default int updateByPrimaryKey(RacPermUrnMo record) {
        return update(c -> c.set(permId).equalTo(record::getPermId).set(urn).equalTo(record::getUrn).where(id, isEqualTo(record::getId)));
    }
    ...
    default int updateByPrimaryKeySelective(RacPermUrnMo record) {
        return update(c -> c.set(permId).equalToWhenPresent(record::getPermId).set(urn).equalToWhenPresent(record::getUrn).where(id, isEqualTo(record::getId)));
    }
    ...
    default int deleteSelective(RacPermUrnMo record) {
        return delete(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(permId, isEqualToWhenPresent(record::getPermId)).and(urn, isEqualToWhenPresent(record::getUrn)));
    }
    ...
    default Optional<RacPermUrnMo> selectOne(RacPermUrnMo record) {
        return selectOne(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(permId, isEqualToWhenPresent(record::getPermId)).and(urn, isEqualToWhenPresent(record::getUrn)));
    }
    ...
    default long countSelective(RacPermUrnMo record) {
        return count(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(permId, isEqualToWhenPresent(record::getPermId)).and(urn, isEqualToWhenPresent(record::getUrn)));
    }
