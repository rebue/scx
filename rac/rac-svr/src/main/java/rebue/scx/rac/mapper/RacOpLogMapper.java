...
public interface RacOpLogMapper extends MapperRootInterface<RacOpLogMo, Long> {
     ...
     */
    BasicColumn[] selectList = BasicColumn.columnList(id, appId, accountId, agentId, opType, opTitle, opDetail, opDatetime);
    ...
    default Long countAccount() {
        SelectStatementProvider countSurvey = SqlBuilder.countFrom(racAccount).build().render(RenderingStrategies.MYBATIS3);
        long count = count(countSurvey);
        return count;
    }
    ...
    default Long countNormalAccount() {
        SelectStatementProvider countSurvey = SqlBuilder.countFrom(racAccount).where(racAccount.isEnabled, isEqualTo(true)).build().render(RenderingStrategies.MYBATIS3);
        long count = count(countSurvey);
        return count;
    }
}
