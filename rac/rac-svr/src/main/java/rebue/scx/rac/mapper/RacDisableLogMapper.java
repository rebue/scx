...
public interface RacDisableLogMapper extends MapperRootInterface<RacDisableLogMo, Long> {
     ...
     */
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "RacDisableLogMoResult", value = { @Result(column = "ID", property = "id", jdbcType = JdbcType.BIGINT, id = true),
        ...
        @Result(column = "ENABLE_OP_ID", property = "enableOpId", jdbcType = JdbcType.BIGINT), @Result(column = "ACCOUNT_ID", property = "accountId", jdbcType = JdbcType.BIGINT),
    ...
    default int insert(RacDisableLogMo record) {
        return MyBatis3Utils.insert(this::insert, record, racDisableLog,
            c -> c.map(id).toProperty("id").map(realmId).toProperty("realmId").map(disableOpAgentId).toProperty("disableOpAgentId").map(enableOpAgentId)
                .toProperty("enableOpAgentId").map(enableOpId).toProperty("enableOpId").map(accountId).toProperty("accountId").map(disableOpId).toProperty("disableOpId")
                .map(disableReason).toProperty("disableReason").map(disableDatetime).toProperty("disableDatetime").map(enableReason).toProperty("enableReason").map(enableDatetime)
                .toProperty("enableDatetime"));
    }
    ...
    default int insertMultiple(Collection<RacDisableLogMo> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, racDisableLog,
            c -> c.map(id).toProperty("id").map(realmId).toProperty("realmId").map(disableOpAgentId).toProperty("disableOpAgentId").map(enableOpAgentId)
                .toProperty("enableOpAgentId").map(enableOpId).toProperty("enableOpId").map(accountId).toProperty("accountId").map(disableOpId).toProperty("disableOpId")
                .map(disableReason).toProperty("disableReason").map(disableDatetime).toProperty("disableDatetime").map(enableReason).toProperty("enableReason").map(enableDatetime)
                .toProperty("enableDatetime"));
    ...
    default int insertSelective(RacDisableLogMo record) {
        return MyBatis3Utils.insert(this::insert, record, racDisableLog,
            c -> c.map(id).toPropertyWhenPresent("id", record::getId).map(realmId).toPropertyWhenPresent("realmId", record::getRealmId).map(disableOpAgentId)
                .toPropertyWhenPresent("disableOpAgentId", record::getDisableOpAgentId).map(enableOpAgentId).toPropertyWhenPresent("enableOpAgentId", record::getEnableOpAgentId)
                .map(enableOpId).toPropertyWhenPresent("enableOpId", record::getEnableOpId).map(accountId).toPropertyWhenPresent("accountId", record::getAccountId).map(disableOpId)
                .toPropertyWhenPresent("disableOpId", record::getDisableOpId).map(disableReason).toPropertyWhenPresent("disableReason", record::getDisableReason)
                .map(disableDatetime).toPropertyWhenPresent("disableDatetime", record::getDisableDatetime).map(enableReason)
                .toPropertyWhenPresent("enableReason", record::getEnableReason).map(enableDatetime).toPropertyWhenPresent("enableDatetime", record::getEnableDatetime));
    }
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(RacDisableLogMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId).set(realmId).equalTo(record::getRealmId).set(disableOpAgentId).equalTo(record::getDisableOpAgentId).set(enableOpAgentId)
            .equalTo(record::getEnableOpAgentId).set(enableOpId).equalTo(record::getEnableOpId).set(accountId).equalTo(record::getAccountId).set(disableOpId)
            .equalTo(record::getDisableOpId).set(disableReason).equalTo(record::getDisableReason).set(disableDatetime).equalTo(record::getDisableDatetime).set(enableReason)
            .equalTo(record::getEnableReason).set(enableDatetime).equalTo(record::getEnableDatetime);
    }
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(RacDisableLogMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId).set(realmId).equalToWhenPresent(record::getRealmId).set(disableOpAgentId)
            .equalToWhenPresent(record::getDisableOpAgentId).set(enableOpAgentId).equalToWhenPresent(record::getEnableOpAgentId).set(enableOpId)
            .equalToWhenPresent(record::getEnableOpId).set(accountId).equalToWhenPresent(record::getAccountId).set(disableOpId).equalToWhenPresent(record::getDisableOpId)
            .set(disableReason).equalToWhenPresent(record::getDisableReason).set(disableDatetime).equalToWhenPresent(record::getDisableDatetime).set(enableReason)
            .equalToWhenPresent(record::getEnableReason).set(enableDatetime).equalToWhenPresent(record::getEnableDatetime);
    }
    ...
    default int updateByPrimaryKey(RacDisableLogMo record) {
        return update(c -> c.set(realmId).equalTo(record::getRealmId).set(disableOpAgentId).equalTo(record::getDisableOpAgentId).set(enableOpAgentId)
            .equalTo(record::getEnableOpAgentId).set(enableOpId).equalTo(record::getEnableOpId).set(accountId).equalTo(record::getAccountId).set(disableOpId)
            .equalTo(record::getDisableOpId).set(disableReason).equalTo(record::getDisableReason).set(disableDatetime).equalTo(record::getDisableDatetime).set(enableReason)
            .equalTo(record::getEnableReason).set(enableDatetime).equalTo(record::getEnableDatetime).where(id, isEqualTo(record::getId)));
    }
    ...
    default int updateByPrimaryKeySelective(RacDisableLogMo record) {
        return update(c -> c.set(realmId).equalToWhenPresent(record::getRealmId).set(disableOpAgentId).equalToWhenPresent(record::getDisableOpAgentId).set(enableOpAgentId)
            .equalToWhenPresent(record::getEnableOpAgentId).set(enableOpId).equalToWhenPresent(record::getEnableOpId).set(accountId).equalToWhenPresent(record::getAccountId)
            .set(disableOpId).equalToWhenPresent(record::getDisableOpId).set(disableReason).equalToWhenPresent(record::getDisableReason).set(disableDatetime)
            .equalToWhenPresent(record::getDisableDatetime).set(enableReason).equalToWhenPresent(record::getEnableReason).set(enableDatetime)
            .equalToWhenPresent(record::getEnableDatetime).where(id, isEqualTo(record::getId)));
    }
    ...
    default int deleteSelective(RacDisableLogMo record) {
        return delete(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(realmId, isEqualToWhenPresent(record::getRealmId))
            .and(disableOpAgentId, isEqualToWhenPresent(record::getDisableOpAgentId)).and(enableOpAgentId, isEqualToWhenPresent(record::getEnableOpAgentId))
            .and(enableOpId, isEqualToWhenPresent(record::getEnableOpId)).and(accountId, isEqualToWhenPresent(record::getAccountId))
            .and(disableOpId, isEqualToWhenPresent(record::getDisableOpId)).and(disableReason, isEqualToWhenPresent(record::getDisableReason))
            .and(disableDatetime, isEqualToWhenPresent(record::getDisableDatetime)).and(enableReason, isEqualToWhenPresent(record::getEnableReason))
