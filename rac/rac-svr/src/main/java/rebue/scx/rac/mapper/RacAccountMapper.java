        ...
        if (qo.getOrgId() != null) {
            if (hierarchical != null) {
                select = SqlBuilder.select(racAccount.allColumns(), racLockLog.allColumns(), racLockLog.id.as("lockLogId")).from(racAccount).rightJoin(racLockLog)
                    ...
                    .where(racAccount.id, isEqualTo(racLockLog.lockAccountId), and(racAccount.realmId, isEqualToWhenPresent(qo::getRealmId)), // and(racOrgAccount.orgId, isEqualToWhenPresent(qo::getOrgId)),
