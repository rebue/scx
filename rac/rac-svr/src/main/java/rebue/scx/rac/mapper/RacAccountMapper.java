...
import static rebue.scx.rac.mapper.RacAccountDynamicSqlSupport.expirationDatetime;
...
public interface RacAccountMapper extends MapperRootInterface<RacAccountMo, Long> {
    ...
    BasicColumn[] selectList = BasicColumn.columnList(id, unionId, userId, remark, orgId, code, realmId, isEnabled, signInName, signInMobile, signInEmail, signInPswd,
        ...
        ddUnionId, ddUserId, ddNickname, ddAvatar, isTester, expirationDatetime, createTimestamp, updateTimestamp);
    ...
    @Results(id = "RacAccountMoResult", value = { @Result(column = "ID", property = "id", jdbcType = JdbcType.BIGINT, id = true),
        ...
        @Result(column = "EXPIRATION_DATETIME", property = "expirationDatetime", jdbcType = JdbcType.TIMESTAMP),
    ...
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @ResultMap("RacAccountMoNonDesensitized")
    Optional<RacAccountNonDesensitizedMo> selectOneNonDesensitizedMo(SelectStatementProvider selectStatement);

    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "RacAccountMoNonDesensitized", value = { @Result(column = "ID", property = "id", jdbcType = JdbcType.BIGINT, id = true),
        @Result(column = "USER_ID", property = "userId", jdbcType = JdbcType.BIGINT), @Result(column = "REMARK", property = "remark", jdbcType = JdbcType.VARCHAR),
        @Result(column = "ORG_ID", property = "orgId", jdbcType = JdbcType.BIGINT), @Result(column = "CODE", property = "code", jdbcType = JdbcType.VARCHAR),
        @Result(column = "REALM_ID", property = "realmId", jdbcType = JdbcType.VARCHAR), @Result(column = "IS_ENABLED", property = "isEnabled", jdbcType = JdbcType.BIT),
        @Result(column = "SIGN_IN_NAME", property = "signInName", jdbcType = JdbcType.VARCHAR),
    ...
    List<RacAccountNonDesensitizedMo> selectManyNonDesensitized(SelectStatementProvider selectStatement);
    ...
    default int insert(RacAccountMo record) {
        return MyBatis3Utils.insert(this::insert, record, racAccount,
            c -> c.map(id).toProperty("id").map(unionId).toProperty("unionId").map(userId).toProperty("userId").map(remark).toProperty("remark").map(orgId).toProperty("orgId")
                .map(code).toProperty("code").map(realmId).toProperty("realmId").map(isEnabled).toProperty("isEnabled").map(signInName).toProperty("signInName").map(signInMobile)
                .toProperty("signInMobile").map(signInEmail).toProperty("signInEmail").map(signInPswd).toProperty("signInPswd").map(signInPswdSalt).toProperty("signInPswdSalt")
                ...
                .toProperty("signInAvatar").map(wxOpenId).toProperty("wxOpenId").map(wxUnionId).toProperty("wxUnionId").map(wxNickname).toProperty("wxNickname").map(wxAvatar)
                ...
                .toProperty("ddNickname").map(ddAvatar).toProperty("ddAvatar").map(isTester).toProperty("isTester").map(expirationDatetime).toProperty("expirationDatetime")
    ...
    default int insertSelective(RacAccountMo record) {
        return MyBatis3Utils.insert(this::insert, record, racAccount,
            c -> c.map(id).toPropertyWhenPresent("id", record::getId).map(unionId).toPropertyWhenPresent("unionId", record::getUnionId).map(userId)
                ...
                .toPropertyWhenPresent("isTester", record::getIsTester).map(expirationDatetime).toPropertyWhenPresent("expirationDatetime", record::getExpirationDatetime)
                .map(createTimestamp).toPropertyWhenPresent("createTimestamp", record::getCreateTimestamp).map(updateTimestamp)
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(RacAccountMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId).set(unionId).equalTo(record::getUnionId).set(userId).equalTo(record::getUserId).set(remark).equalTo(record::getRemark).set(orgId)
            ...
            .set(ddNickname).equalTo(record::getDdNickname).set(ddAvatar).equalTo(record::getDdAvatar).set(isTester).equalTo(record::getIsTester).set(expirationDatetime)
            .equalTo(record::getExpirationDatetime).set(createTimestamp).equalTo(record::getCreateTimestamp).set(updateTimestamp).equalTo(record::getUpdateTimestamp);
    }
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(RacAccountMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId).set(unionId).equalToWhenPresent(record::getUnionId).set(userId).equalToWhenPresent(record::getUserId).set(remark)
            ...
            .set(ddNickname).equalToWhenPresent(record::getDdNickname).set(ddAvatar).equalToWhenPresent(record::getDdAvatar).set(isTester).equalToWhenPresent(record::getIsTester)
            .set(expirationDatetime).equalToWhenPresent(record::getExpirationDatetime).set(createTimestamp).equalToWhenPresent(record::getCreateTimestamp).set(updateTimestamp)
    ...
    default int updateByPrimaryKeySelective(RacAccountMo record) {
        return update(c -> c.set(unionId).equalToWhenPresent(record::getUnionId).set(userId).equalToWhenPresent(record::getUserId).set(remark).equalToWhenPresent(record::getRemark)
            ...
            .equalToWhenPresent(record::getDdAvatar).set(isTester).equalToWhenPresent(record::getIsTester).set(expirationDatetime).equalToWhenPresent(record::getExpirationDatetime)
    ...
    default List<RacAccountMo> list(final RacAccountListTo qo) {
        ...
        if (qo.getOrgId() != null) {
            if (hierarchical != null) {
                SelectStatementProvider select = SqlBuilder.select(racAccount.allColumns(), racUser.realName).from(racAccount).join(racOrgAccount)
                    .on(racAccount.id, equalTo(racOrgAccount.accountId)).leftJoin(racOrg).on(racOrg.id, equalTo(racOrgAccount.orgId)).leftJoin(racUser)
                    .on(racUser.id, equalTo(racAccount.userId))
                    .where(racOrg.treeCode, isLike(hierarchical), and(racAccount.realmId, isEqualToWhenPresent(qo.getRealmId()), sqlCriterion)).groupBy(racAccount.id).build()
                    .render(RenderingStrategies.MYBATIS3);
                return this.selectMany(select);
            }
            else {
                SelectStatementProvider select = SqlBuilder.select(racAccount.allColumns(), racUser.realName).from(racAccount).join(racOrgAccount)
                    .on(racAccount.id, equalTo(racOrgAccount.accountId)).leftJoin(racUser).on(racUser.id, equalTo(racAccount.userId))
                    .where(racOrgAccount.orgId, isEqualTo(qo::getOrgId), and(racAccount.realmId, isEqualToWhenPresent(qo.getRealmId()), sqlCriterion)).groupBy(racAccount.id)
                    .build().render(RenderingStrategies.MYBATIS3);
                return this.selectMany(select);
            }
        }
        else {
            SelectStatementProvider select = SqlBuilder.select(racAccount.allColumns(), racUser.realName).from(racAccount).leftJoin(racUser)
                .on(racUser.id, equalTo(racAccount.userId)).where(racAccount.realmId, isEqualToWhenPresent(qo.getRealmId()), sqlCriterion).groupBy(racAccount.id).build()
                .render(RenderingStrategies.MYBATIS3);
            return this.selectMany(select);
    ...
    default List<RacLockLogAndAccountMo> selectLockAccount(RacAccountPageTo qo, String likeDate) {
        ...
        if (qo.getOrgId() != null) {
            if (hierarchical != null) {
                select = SqlBuilder.select(racAccount.allColumns(), racLockLog.allColumns(), racLockLog.id.as("lockLogId")).from(racAccount).rightJoin(racLockLog)
                    ...
                    .on(racOrg.id, // and(racOrgAccount.orgId,
                        equalTo(racOrgAccount.orgId))
                    .where(// and(racOrgAccount.orgId,
                        racAccount.id, // and(racOrgAccount.orgId,
                        isEqualTo(racLockLog.lockAccountId), // isEqualToWhenPresent(qo::getOrgId)),
                        and(racAccount.realmId, isEqualToWhenPresent(qo::getRealmId)), and(racLockLog.unlockDatetime, SqlBuilder.isNull()),
                        and(racLockLog.column("LOCK_DATETIME"), isLikeWhenPresent(likeDate)), and(racOrg.treeCode, isLike(hierarchical)), sqlCriterion)
                    .build().render(RenderingStrategies.MYBATIS3);
            }
            else {
                select = SqlBuilder.select(racAccount.allColumns(), racLockLog.allColumns(), racLockLog.id.as("lockLogId")).from(racAccount).rightJoin(racLockLog)
            ...
            select = SqlBuilder.select(racAccount.allColumns(), racLockLog.allColumns(), racLockLog.id.as("lockLogId")).from(racAccount).rightJoin(racLockLog)
                ...
                .where(racAccount.id, isEqualTo(racLockLog.lockAccountId), and(racAccount.realmId, isEqualToWhenPresent(qo::getRealmId)),
                    and(racLockLog.unlockDatetime, SqlBuilder.isNull()), and(racLockLog.column("LOCK_DATETIME"), isLikeWhenPresent(likeDate)), sqlCriterion)
                .build().render(RenderingStrategies.MYBATIS3);
        }
    ...
    default int unbindMobile(Long id) {
        UpdateStatementProvider update = SqlBuilder.update(racAccount).set(signInMobile).equalToNull().where(racAccount.id, isEqualTo(id)).build()
            .render(RenderingStrategies.MYBATIS3);
        return this.update(update);
    }
    ...
    default List<RacAccountMo> getAccountByUser(RacAccountPageTo qo) {
        ...
        if (qo.getUserId() != null) {
            select = SqlBuilder.select(racAccount.allColumns()).from(racAccount).rightJoin(racUser).on(racAccount.userId, equalTo(racUser.id))
                .where(racAccount.realmId, isEqualToWhenPresent(qo.getRealmId()), and(racUser.id, isEqualToWhenPresent(qo::getUserId)), sqlCriterion).build()
                .render(RenderingStrategies.MYBATIS3);
        }
        else {
            select = SqlBuilder.select(racAccount.allColumns()).from(racAccount).where(racAccount.realmId, isEqualToWhenPresent(qo.getRealmId()), sqlCriterion).build()
                .render(RenderingStrategies.MYBATIS3);
        }
        return this.selectMany(select);
    }
    ...
    default int setUnionIdIsNull(Long dstId) {
        UpdateStatementProvider update = SqlBuilder.update(racAccount).set(unionId).equalToNull().where(racAccount.id, isEqualTo(dstId)).build()
            .render(RenderingStrategies.MYBATIS3);
        return this.update(update);
    }
     ...
     * @param id 需要解除的账户ID
     */
    default int disassociateUser(Long id) {
        UpdateStatementProvider update = SqlBuilder.update(racAccount).set(userId).equalToNull().where(racAccount.id, isEqualTo(id)).build().render(RenderingStrategies.MYBATIS3);
        return this.update(update);
    }
    ...
    default Optional<RacAccountNonDesensitizedMo> selectByKey(Long accountId) {
        SelectStatementProvider select = SqlBuilder.select(racAccount.allColumns()).from(racAccount).where(racAccount.id, isEqualTo(accountId)).build()
            .render(RenderingStrategies.MYBATIS3);
    ...
    default Optional<RacAccountNonDesensitizedMo> selectByOne(RacAccountMo record) {
        SelectStatementProvider select = SqlBuilder.select(racAccount.allColumns()).from(racAccount).where(id, isEqualToWhenPresent(record::getId))
            .and(unionId, isEqualToWhenPresent(record::getUnionId)).and(userId, isEqualToWhenPresent(record::getUserId)).and(remark, isEqualToWhenPresent(record::getRemark))
            ...
            .and(expirationDatetime, isEqualToWhenPresent(record::getExpirationDatetime)).and(createTimestamp, isEqualToWhenPresent(record::getCreateTimestamp))
            .and(updateTimestamp, isEqualToWhenPresent(record::getUpdateTimestamp)).build().render(RenderingStrategies.MYBATIS3);
        return this.selectOneNonDesensitizedMo(select);
    }
}
