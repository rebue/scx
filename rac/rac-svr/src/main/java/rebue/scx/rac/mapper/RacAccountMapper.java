	...
	default List<RacAccountMo> list(final RacAccountListTo qo) {
		...
		return select(c -> {
			final SqlCriterion<String> sqlCriterion = and(racAccount.signInNickname, isLikeWhenPresent(keywords),
					...
					or(racAccount.id,
							isEqualToWhenPresent(NumberUtils.isValidLong(keywords) ? Long.parseLong(keywords) : null)),
			...
			if (qo.getOrgId() != null) {
				if (hierarchical != null) {
					return c.join(racOrgAccount).on(racAccount.id, equalTo(racOrgAccount.accountId)).leftJoin(racOrg)
							.on(racOrg.id, equalTo(racOrgAccount.orgId)).where(racOrg.treeCode, isLike(hierarchical),
									and(racAccount.domainId, isEqualToWhenPresent(qo.getDomainId()), sqlCriterion));
				} else {
					return c.join(racOrgAccount).on(racAccount.id, equalTo(racOrgAccount.accountId)).where(
							racOrgAccount.orgId, isEqualTo(qo::getOrgId),
							and(racAccount.domainId, isEqualToWhenPresent(qo.getDomainId()), sqlCriterion));
				}
			} else {
