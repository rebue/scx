        ...
        if (qo.getOrgId() != null) {
            if (hierarchical != null) {
                select = SqlBuilder.select(racAccount.allColumns(), racLockLog.allColumns(), racLockLog.id.as("lockLogId")).from(racAccount).rightJoin(racLockLog)
                    ...
                    .on(racOrg.id, equalTo(racOrgAccount.orgId)).where(// and(racOrgAccount.orgId,
                        racAccount.id, // and(racOrgAccount.orgId,
                        isEqualTo(racLockLog.lockAccountId), // and(racOrgAccount.orgId,
                        and(racAccount.realmId, isEqualToWhenPresent(qo::getRealmId)), // isEqualToWhenPresent(qo::getOrgId)),
                        and(racLockLog.unlockDatetime, SqlBuilder.isNull()), and(racLockLog.column("LOCK_DATETIME"), isLikeWhenPresent(likeDate)),
                        and(racOrg.treeCode, isLike(hierarchical)), sqlCriterion)
                    .build().render(RenderingStrategies.MYBATIS3);
            }
            else {
                select = SqlBuilder.select(racAccount.allColumns(), racLockLog.allColumns(), racLockLog.id.as("lockLogId")).from(racAccount).rightJoin(racLockLog)
