...
public interface RacAccountMapper extends MapperRootInterface<RacAccountMo, Long> {
        ...
        if (qo.getOrgId() != null) {
            if (hierarchical != null) {
                select = SqlBuilder.select(racAccount.allColumns(), racLockLog.allColumns(), racLockLog.id.as("lockLogId")).from(racAccount).rightJoin(racLockLog)
                        ...
                        .where(racAccount.id, isEqualTo(racLockLog.lockAccountId), and(racAccount.realmId, isEqualToWhenPresent(qo::getRealmId)), // and(racOrgAccount.orgId,
                                                                                                                                                  // isEqualToWhenPresent(qo::getOrgId)),
    ...
    default int disassociateUser(Long id) {
        UpdateStatementProvider update = SqlBuilder.update(racAccount).set(userId).equalToNull().where(racAccount.id, isEqualTo(id)).build()
                .render(RenderingStrategies.MYBATIS3);
        return this.update(update);
    }
}
