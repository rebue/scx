...
public interface RacAccountMapper extends MapperRootInterface<RacAccountMo, Long> {
	...
	default int insert(final RacAccountMo record) {
		return MyBatis3Utils.insert(this::insert, record, racAccount,
				c -> c.map(id).toProperty("id").map(userId).toProperty("userId").map(remark).toProperty("remark").map(orgId).toProperty("orgId").map(domainId)
						.toProperty("domainId")
	...
	default int updateByPrimaryKeySelective(final RacAccountMo record) {
		return update(c -> c.set(userId).equalToWhenPresent(record::getUserId).set(remark).equalToWhenPresent(record::getRemark).set(orgId).equalToWhenPresent(record::getOrgId)
				.set(domainId).equalToWhenPresent(record::getDomainId).set(isEnabled).equalToWhenPresent(record::getIsEnabled).set(signInName)
	...
	default long countSelective(final RacAccountMo record) {
		return count(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(userId, isEqualToWhenPresent(record::getUserId)).and(remark, isEqualToWhenPresent(record::getRemark))
				.and(orgId, isEqualToWhenPresent(record::getOrgId)).and(domainId, isEqualToWhenPresent(record::getDomainId))
	 ...
	 */
	@Override
	default List<RacAccountMo> selectIn(final List<Long> ids) {
