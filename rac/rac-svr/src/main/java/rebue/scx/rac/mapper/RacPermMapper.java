...
public interface RacPermMapper extends MapperRootInterface<RacPermMo, Long> {
	...
	default int insert(RacPermMo record) {
		return MyBatis3Utils.insert(this::insert, record, racPerm,
				c -> c.map(id).toProperty("id").map(domainId).toProperty("domainId").map(groupId).toProperty("groupId")
						.map(name).toProperty("name").map(isEnabled).toProperty("isEnabled").map(seqNo)
						.toProperty("seqNo").map(remark).toProperty("remark"));
	}
	...
	default int insertMultiple(Collection<RacPermMo> records) {
		return MyBatis3Utils.insertMultiple(this::insertMultiple, records, racPerm,
				c -> c.map(id).toProperty("id").map(domainId).toProperty("domainId").map(groupId).toProperty("groupId")
						.map(name).toProperty("name").map(isEnabled).toProperty("isEnabled").map(seqNo)
						.toProperty("seqNo").map(remark).toProperty("remark"));
	}
	...
	default int insertSelective(RacPermMo record) {
		return MyBatis3Utils.insert(this::insert, record, racPerm,
				c -> c.map(id).toPropertyWhenPresent("id", record::getId).map(domainId)
						.toPropertyWhenPresent("domainId", record::getDomainId).map(groupId)
						.toPropertyWhenPresent("groupId", record::getGroupId).map(name)
	...
	static UpdateDSL<UpdateModel> updateAllColumns(RacPermMo record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(id).equalTo(record::getId).set(domainId).equalTo(record::getDomainId).set(groupId)
				.equalTo(record::getGroupId).set(name).equalTo(record::getName).set(isEnabled)
				.equalTo(record::getIsEnabled).set(seqNo).equalTo(record::getSeqNo).set(remark)
	...
	static UpdateDSL<UpdateModel> updateSelectiveColumns(RacPermMo record, UpdateDSL<UpdateModel> dsl) {
		return dsl.set(id).equalToWhenPresent(record::getId).set(domainId).equalToWhenPresent(record::getDomainId)
				.set(groupId).equalToWhenPresent(record::getGroupId).set(name).equalToWhenPresent(record::getName)
				.set(isEnabled).equalToWhenPresent(record::getIsEnabled).set(seqNo).equalToWhenPresent(record::getSeqNo)
	...
	default int updateByPrimaryKey(RacPermMo record) {
		return update(c -> c.set(domainId).equalTo(record::getDomainId).set(groupId).equalTo(record::getGroupId)
				.set(name).equalTo(record::getName).set(isEnabled).equalTo(record::getIsEnabled).set(seqNo)
	...
	default int updateByPrimaryKeySelective(RacPermMo record) {
		return update(c -> c.set(domainId).equalToWhenPresent(record::getDomainId).set(groupId)
				.equalToWhenPresent(record::getGroupId).set(name).equalToWhenPresent(record::getName).set(isEnabled)
				.equalToWhenPresent(record::getIsEnabled).set(seqNo).equalToWhenPresent(record::getSeqNo).set(remark)
	...
	default int deleteSelective(RacPermMo record) {
		return delete(c -> c.where(id, isEqualToWhenPresent(record::getId))
				...
				.and(groupId, isEqualToWhenPresent(record::getGroupId)).and(name, isEqualToWhenPresent(record::getName))
