...
public class JwtPreFilter extends ZuulFilter {
    ...
    public String filterType() {
        log.info("设置JwtPreFilter的过滤器类型为pre");
        return "pre";
    }

    @Override
    public int filterOrder() {
        log.info("设置JwtPreFilter的排序号为{}(数字越大，优先级越高)", filterOrder);
        return filterOrder;
    }

    @Override
    public boolean shouldFilter() {
        log.info("设置是否需要执行JwtPreFilter过滤器: {}", shouldFilter);
        return shouldFilter;
    }
