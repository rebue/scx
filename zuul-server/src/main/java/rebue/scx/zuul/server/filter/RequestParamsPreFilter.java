...
public class RequestParamsPreFilter extends ZuulFilter {
    ...
    public String filterType() {
        log.info("设置RequestParamsPreFilter的过滤器类型为pre");
        return "pre";
    }

    @Override
    public int filterOrder() {
        log.info("设置RequestParamsPreFilter的排序号为{}(数字越大，优先级越高)", filterOrder);
        return filterOrder;
    }

    @Override
    public boolean shouldFilter() {
        log.info("设置是否需要执行RequestParamsPreFilter过滤器: {}", shouldFilter);
        return shouldFilter;
    }
        ...
        try {
            ...
            try {
                ...
                if (!StringUtils.isBlank(body)) {
                    log.debug("请求参数类型是Body，将其加入到ctx中传递给其它过滤器: {}", body);
                ...
                final String msg = "获取请求body出现异常";
                log.error(msg, e);
                throw new RuntimeException(e);
            }

            log.debug("判断请求参数是否是Query");
            final Map<String, String[]> oldParams = req.getParameterMap();
            if (oldParams != null && !oldParams.isEmpty()) {
                final Map<String, List<String>> newParams = new HashMap<>();
                for (final Entry<String, String[]> entry : oldParams.entrySet()) {
                    final String[] oldValues = entry.getValue();
                ...
                log.debug("请求参数类型是Query，将其加入到ctx中传递给其它过滤器: {}", newParams);
            ...
            log.debug("没有请求参数");
            log.debug("请求参数类型是NONE，将其加入到ctx中传递给其它过滤器");
