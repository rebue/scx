...
public interface WxxMchMapper extends MapperRootInterface<WxxMchMo, String> {
     ...
     */
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "WxxMchMoResult", value = { @Result(column = "ID", property = "id", jdbcType = JdbcType.VARCHAR, id = true),
        @Result(column = "NAME", property = "name", jdbcType = JdbcType.VARCHAR), @Result(column = "API_KEY", property = "apiKey", jdbcType = JdbcType.VARCHAR)
    })
    ...
    default int insert(WxxMchMo record) {
        return MyBatis3Utils.insert(this::insert, record, wxxMch, c -> c.map(id).toProperty("id").map(name).toProperty("name").map(apiKey).toProperty("apiKey"));
    }
    ...
    default int insertMultiple(Collection<WxxMchMo> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, wxxMch, c -> c.map(id).toProperty("id").map(name).toProperty("name").map(apiKey).toProperty("apiKey"));
    }
    ...
    default int insertSelective(WxxMchMo record) {
        return MyBatis3Utils.insert(this::insert, record, wxxMch, c -> c.map(id).toPropertyWhenPresent("id", record::getId).map(name).toPropertyWhenPresent("name", record::getName)
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(WxxMchMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId).set(name).equalTo(record::getName).set(apiKey).equalTo(record::getApiKey);
    }
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(WxxMchMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId).set(name).equalToWhenPresent(record::getName).set(apiKey).equalToWhenPresent(record::getApiKey);
    }
    ...
    default int updateByPrimaryKey(WxxMchMo record) {
        return update(c -> c.set(name).equalTo(record::getName).set(apiKey).equalTo(record::getApiKey).where(id, isEqualTo(record::getId)));
    }
    ...
    default int updateByPrimaryKeySelective(WxxMchMo record) {
        return update(c -> c.set(name).equalToWhenPresent(record::getName).set(apiKey).equalToWhenPresent(record::getApiKey).where(id, isEqualTo(record::getId)));
    }
    ...
    default int deleteSelective(WxxMchMo record) {
        return delete(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(name, isEqualToWhenPresent(record::getName)).and(apiKey, isEqualToWhenPresent(record::getApiKey)));
    }
    ...
    default Optional<WxxMchMo> selectOne(WxxMchMo record) {
        return selectOne(
            c -> c.where(id, isEqualToWhenPresent(record::getId)).and(name, isEqualToWhenPresent(record::getName)).and(apiKey, isEqualToWhenPresent(record::getApiKey)));
    }
    ...
    default long countSelective(WxxMchMo record) {
        return count(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(name, isEqualToWhenPresent(record::getName)).and(apiKey, isEqualToWhenPresent(record::getApiKey)));
    }
    ...
    default List<WxxMchMo> selectSelective(WxxMchMo record) {
        return select(c -> c.where(id, isEqualToWhenPresent(record::getId)).and(name, isEqualToWhenPresent(record::getName)).and(apiKey, isEqualToWhenPresent(record::getApiKey)));
    }
     ...
     * @mbg.generated 自动生成，如需修改，请删除本行
     */
    default List<WxxMchMo> selectIn(List<String> ids) {
