...
public interface WxxAppMapper extends MapperRootInterface<WxxAppMo, String> {
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
     */
    BasicColumn[] selectList = BasicColumn.columnList(id, name, mchId, appSecret, token, encodeingAesKey, subscribeAutoReply, menu, loginCallbackUrl, loginCallbackMethodType, loginCallbackSignkey, wxpayNotifyUrl);
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
     */
    @SelectProvider(type=SqlProviderAdapter.class, method="select")
    @Results(id="WxxAppMoResult", value = {
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
    ...
    default int insert(WxxAppMo record) {
        return MyBatis3Utils.insert(this::insert, record, wxxApp, c ->
            c.map(id).toProperty("id")
            .map(name).toProperty("name")
            .map(mchId).toProperty("mchId")
            .map(appSecret).toProperty("appSecret")
            .map(token).toProperty("token")
            .map(encodeingAesKey).toProperty("encodeingAesKey")
            .map(subscribeAutoReply).toProperty("subscribeAutoReply")
            .map(menu).toProperty("menu")
            .map(loginCallbackUrl).toProperty("loginCallbackUrl")
            .map(loginCallbackMethodType).toProperty("loginCallbackMethodType")
            .map(loginCallbackSignkey).toProperty("loginCallbackSignkey")
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
    ...
    default int insertMultiple(Collection<WxxAppMo> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, wxxApp, c ->
            c.map(id).toProperty("id")
            .map(name).toProperty("name")
            .map(mchId).toProperty("mchId")
            .map(appSecret).toProperty("appSecret")
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
    ...
    default int insertSelective(WxxAppMo record) {
        return MyBatis3Utils.insert(this::insert, record, wxxApp, c ->
            ...
            .map(mchId).toPropertyWhenPresent("mchId", record::getMchId)
            .map(appSecret).toPropertyWhenPresent("appSecret", record::getAppSecret)
            .map(token).toPropertyWhenPresent("token", record::getToken)
            .map(encodeingAesKey).toPropertyWhenPresent("encodeingAesKey", record::getEncodeingAesKey)
            .map(subscribeAutoReply).toPropertyWhenPresent("subscribeAutoReply", record::getSubscribeAutoReply)
            .map(menu).toPropertyWhenPresent("menu", record::getMenu)
            .map(loginCallbackUrl).toPropertyWhenPresent("loginCallbackUrl", record::getLoginCallbackUrl)
            .map(loginCallbackMethodType).toPropertyWhenPresent("loginCallbackMethodType", record::getLoginCallbackMethodType)
            .map(loginCallbackSignkey).toPropertyWhenPresent("loginCallbackSignkey", record::getLoginCallbackSignkey)
            .map(wxpayNotifyUrl).toPropertyWhenPresent("wxpayNotifyUrl", record::getWxpayNotifyUrl)
        );
    }
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(WxxAppMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId)
                ...
                .set(mchId).equalTo(record::getMchId)
                .set(appSecret).equalTo(record::getAppSecret)
                .set(token).equalTo(record::getToken)
                .set(encodeingAesKey).equalTo(record::getEncodeingAesKey)
                .set(subscribeAutoReply).equalTo(record::getSubscribeAutoReply)
                .set(menu).equalTo(record::getMenu)
                .set(loginCallbackUrl).equalTo(record::getLoginCallbackUrl)
                .set(loginCallbackMethodType).equalTo(record::getLoginCallbackMethodType)
                .set(loginCallbackSignkey).equalTo(record::getLoginCallbackSignkey)
                .set(wxpayNotifyUrl).equalTo(record::getWxpayNotifyUrl);
    }
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(WxxAppMo record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId)
                ...
                .set(mchId).equalToWhenPresent(record::getMchId)
                .set(appSecret).equalToWhenPresent(record::getAppSecret)
                .set(token).equalToWhenPresent(record::getToken)
                .set(encodeingAesKey).equalToWhenPresent(record::getEncodeingAesKey)
                .set(subscribeAutoReply).equalToWhenPresent(record::getSubscribeAutoReply)
                .set(menu).equalToWhenPresent(record::getMenu)
                .set(loginCallbackUrl).equalToWhenPresent(record::getLoginCallbackUrl)
                .set(loginCallbackMethodType).equalToWhenPresent(record::getLoginCallbackMethodType)
                .set(loginCallbackSignkey).equalToWhenPresent(record::getLoginCallbackSignkey)
                .set(wxpayNotifyUrl).equalToWhenPresent(record::getWxpayNotifyUrl);
    }
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
    ...
    default int deleteSelective(WxxAppMo record) {
        return delete(c ->
            ...
            .and(mchId, isEqualToWhenPresent(record::getMchId))
            .and(appSecret, isEqualToWhenPresent(record::getAppSecret))
            .and(token, isEqualToWhenPresent(record::getToken))
            .and(encodeingAesKey, isEqualToWhenPresent(record::getEncodeingAesKey))
            .and(subscribeAutoReply, isEqualToWhenPresent(record::getSubscribeAutoReply))
            .and(menu, isEqualToWhenPresent(record::getMenu))
            .and(loginCallbackUrl, isEqualToWhenPresent(record::getLoginCallbackUrl))
            .and(loginCallbackMethodType, isEqualToWhenPresent(record::getLoginCallbackMethodType))
            .and(loginCallbackSignkey, isEqualToWhenPresent(record::getLoginCallbackSignkey))
